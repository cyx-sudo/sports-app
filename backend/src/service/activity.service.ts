...
export class ActivityService {
  ...
  // 获取活动列表
  async getActivityList(params: ActivityListRequest): Promise<ActivityListResponse> {
    const db = this.databaseService.getDatabase();
    ...
    if (params.search) {
      whereClause += ' AND (name LIKE ? OR description LIKE ? OR instructor LIKE ?)';
    ...
    const listQuery = `
      ...
      LIMIT ? OFFSET ?
    `;
    const activities = db.prepare(listQuery).all(...queryParams, limit, offset) as Activity[];
  ...
  // 更新活动
  async updateActivity(id: number, updateData: UpdateActivityRequest): Promise<Activity | null> {
    const db = this.databaseService.getDatabase();
  ...
  async getActivityCategories(): Promise<string[]> {
    ...
    const categories = db.prepare('SELECT DISTINCT category FROM activities ORDER BY category').all() as { category: string }[];
