...
export class UserService {
  ...
  async register(userData: RegisterRequest): Promise<{ success: boolean; message: string }> {
    ...
    // 检查手机号是否已存在
    const existingPhone = db.prepare('SELECT id FROM users WHERE phone = ?').get(userData.phone);
    if (existingPhone) {
    ...
    try {
      insertUser.run(userData.username, hashedPassword, userData.email, userData.phone, userData.realName);
  ...
  async login(loginData: LoginRequest): Promise<LoginResponse> {
    ...
    // 查找用户
    const user = db.prepare('SELECT * FROM users WHERE username = ?').get(loginData.username) as User;
    if (!user) {
  ...
  async getAllUsers(): Promise<Omit<User, 'password'>[]> {
    const db = this.databaseService.getDatabase();
    const users = db.prepare('SELECT id, username, email, phone, realName, createdAt, updatedAt FROM users').all() as Omit<User, 'password'>[];
    return users;
  }

  // 根据 ID 获取用户
  async getUser(options: { uid: number }): Promise<Omit<User, 'password'> | null> {
    const db = this.databaseService.getDatabase();
    const user = db.prepare('SELECT id, username, email, phone, realName, createdAt, updatedAt FROM users WHERE id = ?').get(options.uid) as Omit<User, 'password'>;
    return user || null;
  }
}
